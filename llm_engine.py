...
class LLMEngine:
    def __init__(self, api_key: str | None = None):
        # 1. Resolve key priority: Explicit argument -> Streamlit Secrets / Env
        active_key = api_key or groq_api_key
        
        if not active_key:
            raise ValueError("GROQ_API_KEY is missing. Please check your .env file or Streamlit Secrets.")

        # 2. Wire the internal client directly to Groq SDK
